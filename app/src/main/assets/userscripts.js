...
//
// BUG (fixed): this used to overwrite the <meta name="viewport"> tag with
// "width=3840, height=2160, initial-scale=1.0". That forces the CSS *layout*
// viewport to a fixed 3840px width while telling the WebView the content is
// simultaneously at initial-scale=1.0. Android's WebView (with
// useWideViewPort=true) resolves that contradiction by rescaling the already
// rendered page so that 3840 CSS px fit into the real, much narrower on-screen
// width once layout is finalized (onPageFinished). Because the actual CSS
// width of the WebView on most TV boxes is tiny compared to 3840, the
// computed zoom factor explodes (observed around ~19000%, i.e. ~190x zoom).
// "height" is also not a valid viewport descriptor and only adds to the
// confusion.
//
// Fix: never touch the layout viewport for this. YouTube TV's web client
// decides the UI resolution tier from window.screen / devicePixelRatio, not
// from the layout viewport meta tag. So we spoof those instead, and leave the
// viewport meta as "width=device-width" (matching the native WebView
// settings), which keeps scale at a normal 100% both during and after load.
(function () {
  if (window.__notube_spoof_4k_applied) return;
  window.__notube_spoof_4k_applied = true;

  var TARGET_WIDTH = 3840;
  var TARGET_HEIGHT = 2160;
  ...
  function defineReadOnly(obj, prop, value) {
    try {
      ...
      // Some WebView builds may not allow redefining certain screen
      // properties; fail silently rather than breaking the page.
    }
  }

  try {
    if (window.screen) {
      defineReadOnly(window.screen, "width", TARGET_WIDTH);
      defineReadOnly(window.screen, "height", TARGET_HEIGHT);
      defineReadOnly(window.screen, "availWidth", TARGET_WIDTH);
      defineReadOnly(window.screen, "availHeight", TARGET_HEIGHT);
    }
    // devicePixelRatio is left untouched on purpose: changing it affects how
    // the WebView maps CSS px to physical px and can itself cause zoom-like
    // rendering artifacts. YouTube TV's 4K UI detection only needs
    // window.screen's dimensions.

    // Ensure the layout viewport itself stays sane (device-width, no forced
    // zoom), matching the native useWideViewPort/loadWithOverviewMode
    // settings configured on the Kotlin side. This does NOT affect the
    // window.screen spoofing above.
...
/* Shared helper: coalesce a MutationObserver callback to run at most once
   per animation frame, no matter how many mutation records fire in that
   frame. YouTube TV's DOM churns very heavily (focus/scroll animations,
   list virtualization), so document-wide observers that recompute on every
   single mutation record are a major source of jank on TV hardware. This
   keeps the same behavior/timing granularity a user can perceive (sub-frame
   duplicate work is invisible) while cutting the number of expensive
   querySelector scans drastically. */
function __notubeRafThrottle(fn) {
  var scheduled = false;
  return function () {
    if (scheduled) return;
    scheduled = true;
...
(function () {
  const observer = new MutationObserver(
    __notubeRafThrottle(function () {
      ...
      // BUG (fixed): this had no guard, so every single DOM mutation
      // anywhere on the page (subtree:true on document.body) re-attached a
      // brand new keydown listener to the same exit button as long as it
      // stayed in the DOM. On a heavily animated SPA like YouTube TV that
      // meant dozens of duplicate listeners piling up per second, which both
      // wasted CPU (a growing source of lag over a session) and would fire
      // ExitBridge.onExitCalled() multiple times per Enter press. A simple
      // one-time-bind guard fixes both.
      if (exitButton.dataset.notubetvExitBound === "1") return;
      exitButton.dataset.notubetvExitBound = "1";
...
(function () {
  ...
  window.configWrite = function (key, value) {
    ...
    try {
      window.localStorage.setItem(CONFIG_KEY, JSON.stringify(window.localConfig));
    } catch (err) {
      // Storage may be unavailable in a restricted WebView; keep the in-memory value.
    }
  };
  ...
  class SponsorBlockHandler {
    ...
    async init() {
      ...
      const resp = await new Promise((resolve) => {
        ...
        window.onNetworkBridgeResponse = (jsonString) => finish(jsonString);
        NetworkBridge.fetch(
      ...
      let result;
      try {
        result = resp ? JSON.parse(resp) : null;
      } catch (err) {
        result = null;
      }
