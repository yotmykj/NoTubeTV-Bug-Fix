...
object UpdateManager {
    ...
    suspend fun checkAppUpdate(context: Context, navigator: WebViewNavigator?, callback: (ReleaseData?) -> Unit) {
        try {
            ...
            if (isNewerVersion(remote = remoteVersion, local = localVersion)) {
                val skipVersion = getSkipVersion(context)
    ...
    private fun getSkipVersion(context: Context): String? {
        val prefs = context.getSharedPreferences("notube_settings", Context.MODE_PRIVATE)
        return prefs.getString("skipVersionName", null)
    }
}
